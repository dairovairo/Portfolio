...
function PoolCard({ pool, onJoin, onLeave, onCancel, onOpenDetail, onToast, joining, leaving }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="mt-2 flex items-start gap-2 flex-wrap"
        ...
        {pool.participant_count === 0 ? (
          ...
          <>
            {(pool.participants_preview || []).slice(0, PARTICIPANT_MASCOTS_VISIBLE).map(p => (
              <div key={p.id} className="flex flex-col items-center flex-shrink-0" style={{ width: 48 }} title={p.username}>
                <div style={{ marginTop: -4 }}>
                  <MiniMascot user={p} size={41} />
                </div>
                <span className="text-[9px] font-display font-semibold text-surface-muted mt-0.5 max-w-[48px] truncate">
                  {p.mascot_name || 'Volty'}
                </span>
              </div>
            ))}
            {pool.participant_count > PARTICIPANT_MASCOTS_VISIBLE && (
              <div className="flex flex-col items-center justify-center flex-shrink-0" style={{ width: 41, height: 41, marginTop: -4 }}>
...
export default function PoolsPage() {
  ...
  // Entrar en el menú Quedadas = "he visto las notificaciones de quedadas
  // nuevas" → limpia el badge del dock inferior y el del tab "Activos" a la
  // vez (ambos leen poolInviteBadgeCount del mismo contexto).
  useEffect(() => {
  ...
  // Nº de notificaciones de quedadas nuevas sin ver (públicas de amigos +
  // invitaciones privadas) — badge en el tab "Activos", que comparte fuente
  // con el badge del dock inferior (ver PoolInviteNotificationsContext) para
  // que ambos muestren siempre el mismo número y se limpien a la vez.
