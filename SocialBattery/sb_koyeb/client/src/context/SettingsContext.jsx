...
export function SettingsProvider({ children }) {
  ...
  const setMuteNewRaffles = useCallback((v) => {
    ...
    // El aviso de "nuevo sorteo en tu comunidad" (avioneta banner) es un
    // web-push real que manda el servidor (notifyCommunityRaffleTargets en
    // routes/community.js, fase 107) y que llega igual con la app en
    // foreground o en segundo plano/cerrada, así que este ajuste tiene que
    // persistir en el usuario (users.mute_new_raffles) — no basta con
    // localStorage. Mismo patrón que mute_new_events / mute_new_pools.
    import('../lib/api').then(({ api }) => {
