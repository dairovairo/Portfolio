...
try {
  ...
  fail('La URL configurada no es valida. Ejecuta: npm run set-url -- https://tu-app.onrender.com');
}
...
if (parsed.protocol !== 'https:' || placeholderWords.some(word => parsed.hostname.toLowerCase().includes(word))) {
  fail('Configura primero la URL real de Render: npm run set-url -- https://tu-app.onrender.com');
}
...
(async () => {
  ...
  try {
    const response = await fetch(parsed.href, {
      ...
      headers: { 'user-agent': 'SocialBattery mobile build check' },
    });
